...
// 销售订单
exports.mgDinpds = (req, res) => {
	let crUser = req.session.crUser;

	res.render('./user/mger/ordin/dinpd/list', {
		title: '销售订单',
		crUser,
	})
}

exports.mgDinpd = (req, res) => {
	let crUser = req.session.crUser;
	...
	.exec((err, dinpd) => {
		if(err) {
			console.log(err);
			info = "mger Dinpd, Compd.findOne, Error!";
			...
			// console.log(dinpd)
			res.render('./user/mger/ordin/dinpd/detail', {
				title: '销售订单详情',
				crUser,
				dinpd
			})
...
exports.mgDinpdUp = (req, res) => {
	...
	.exec((err, dinpd) => {
		if(err) {
			...
			res.render('./user/mger/ordin/dinpd/update', {
				title: '销售订单修改',
				crUser,
				dinpd,
			})
...
exports.mgDinpdDel = (req, res) => {
	...
	.exec((err, compd) => {
		if(err) {
			console.log(err);
			info = "mger DinpdDel, Compd.findOne, Error!";
			Err.usError(req, res, info);
		} else if(!compd) {
			info = "这个销售订单已经被删除";
			Err.usError(req, res, info);
...
exports.mgDinpdUpdAjax = (req, res) => {
	let crUser = req.session.crUser;
	...
	.exec((err, compd) => {
		if(err) {
			console.log(err);
			info = "mger DinpdUpd, Strmup.findOne, Error!"
			Err.jsonErr(req, res, info);
		} else if(!compd) {
			info = 'mger DinpdUpdAjax 此产品已经被删除, 请截图后, 联系管理员';
			Err.jsonErr(req, res, info);
		} else if(!compd.ordin) {
			info = 'mger DinpdUpdAjax 此产品所属销售订单已经被删除, 请截图后, 联系管理员';
			Err.jsonErr(req, res, info);
		} else if(compd.ordin.status != Conf.status.init.num) {
			info = 'mger DinpdUpdAjax 此产品所属销售订单状态已经改变, 价格不可更改';
			Err.jsonErr(req, res, info);
		} else {
...
exports.mgDinOptpd = (req, res) => {
	...
	Ordin.findOne({_id: dinId}, (err, din) => {
		if(err) {
			console.log(err);
			info = "mger Din option pd, Ordin.findOne, Error!"
			...
			Inquot.findOne({_id: inquotId}, (err, qun) => {
				if(err) {
					console.log(err);
					info = "mger Din option pd, Inquot.findOne, Error!"
					Err.usError(res, req, info);
				} else if(!qun) {
					...
					Compd.findOne({_id: compdId}, (err, qunpd) => {
						if(err) {
							console.log(err);
							info = "mger Din option pd, Compd.findOne, Error!"
							Err.usError(res, req, info);
						} else if(!qunpd) {
							...
							_compd.save((err, pdSave) => {
								if(err) {
									console.log(err);
									info = "mger Din option pd, _compd.save, Error!"
									Err.usError(res, req, info);
								} else {
									qunpd.shelf = 1
									qunpd.save((err, qunpdSave) => {
										if(err) console.log(err);
										res.redirect("/mgQunGenDin?inquotId="+inquotId+"&ordinId="+dinId)
									})
								}
							})
						}
